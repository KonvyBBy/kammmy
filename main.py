    ...
    if os.path.exists(DB_FILE):
        with open(DB_FILE, 'r') as f:
            try:
                ...
                pass
    return {"stock": {}, "sent_keys": [], "reminders": [], "tickets": {}, "resellers": []}
...
def is_authorized():
    ...
    async def predicate(interaction: discord.Interaction):
        ...
        return interaction.user.id in db.get("resellers", [])
...
@bot.tree.command(name="addreseller", description="Add a reseller (Owner only)")
...
async def addreseller(interaction: discord.Interaction, user: discord.User):
    ...
    if user.id == CONFIG["OWNER_ID"]:
        await interaction.response.send_message(
            f"{CONFIG['EMOJIS']['cross']} The owner doesn't need to be added as a reseller!",
            ephemeral=True)
        return
    ...
    embed = create_styled_embed(
        f"{CONFIG['EMOJIS']['check']} Reseller Added",
        f"**User {CONFIG['EMOJIS']['user']}** {user.mention} ({user.id})\n\nThey can now use all bot commands."
    )
    ...
    await log_to_webhook(
        ...
        description=f"New reseller granted access",
        color=0x00ff00,
        fields=[
            {"name": "Reseller", "value": f"{user.mention} ({user.id})", "inline": True},
            {"name": "Added By", "value": f"{interaction.user.mention} ({interaction.user.id})", "inline": True}
        ]
    )


@bot.tree.command(name="removereseller", description="Remove a reseller (Owner only)")
...
async def removereseller(interaction: discord.Interaction, user: discord.User):
    ...
    embed = create_styled_embed(
        f"{CONFIG['EMOJIS']['check']} Reseller Removed",
    ...
    await log_to_webhook(
        ...
        description=f"Reseller access revoked",
        color=0xff0000,
        fields=[
            {"name": "Reseller", "value": f"{user.mention} ({user.id})", "inline": True},
            {"name": "Removed By", "value": f"{interaction.user.mention} ({interaction.user.id})", "inline": True}
        ]
    )


@bot.tree.command(name="listresellers", description="List all resellers (Owner only)")
@is_owner()
async def listresellers(interaction: discord.Interaction):
    resellers = db.get("resellers", [])
    
    if not resellers:
        desc = "No resellers added yet."
    else:
        reseller_list = []
    ...
    embed = create_styled_embed(
        f"{CONFIG['EMOJIS']['user']} Reseller List ({len(resellers)})",
        desc
    )
    await interaction.response.send_message(embed=embed, ephemeral=True)
    ...
    # Log to webhook
    await log_to_webhook(
        title="📋 Resellers Listed",
        description="Reseller list accessed",
        color=0x0099ff,
        fields=[
            {"name": "Total Resellers", "value": str(len(resellers)), "inline": True},
    ...
    if isinstance(error, app_commands.CheckFailure):
        if not interaction.response.is_done():
            ...
            if interaction.command and interaction.command.name in ['addreseller', 'removereseller', 'listresellers']:
                ...
                await interaction.response.send_message(
                    f"{CONFIG['EMOJIS']['cross']} **Authorization Failed!**\nYour ID: `{user_id}` is not authorized. Only the owner and resellers can use this command.",
                    ephemeral=True)
    else:
